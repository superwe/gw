...
class Login_model extends CI_Model{
    ...
    */
     ...
     * @return array
     */
    public function getUserInfo($user, $spaceid = 0)
    {
    	$spaceid = $spaceid ? $spaceid : 'p.defaultspaceid';
        $query = $this->db->query('select p.id,p.oldpwd,p.password,p.oldsalt,p.salt,s.id as spaceid,e.id as eid,e.name,e.imageurl,
                                  s.name as sname,s.logourl,s.groupid,s.type from tb_person as p
                                  left join tb_employee as e on p.id=e.personid and '.$spaceid.'=e.spaceid
                                  left join tb_space as s on '.$spaceid.'=s.id
                                  where p.email=?',array($user));
        $data=[];
        if($query->num_rows() > 0)
        {
            $row = $query->row();
            ...
            $data['salt']=$row->salt;
            $data['spaceId']=$row->spaceid;
            $data['employeeId']=$row->eid;
            $data['employeeName']=$row->name;
            $data['employeeLogo']=$row->imageurl;
            $data['spaceName']=$row->sname;
            $data['spaceLogo']=$row->logourl;
    ...
    {
        $query = $this->db->query('select id,name from tb_region where level=0');
        return json_encode($query->result_array());
    }

    public function getRegionById($parentid)
    {
        $query = $this->db->query('select id,name from tb_region where level=1 and parentid=?',array($parentid));
        return json_encode($query->result_array());
    }
    ...
    {
        return $this->app->getAppList($spaceId);
    }
    ...
    {
        $query=$this->db->query('select g.id,g.name,g.logourl from tb_group as g
                                                        where g.status=1 and g.id in (select groupid from tb_group_employee
                                                        where status!=0 and employeeid=?) ORDER BY g.createtime DESC LIMIT 5 offset 0',array($id));
     ...
     * @return array
     */
    public function getInnerSpaceList($id)
    {
        $query=$this->db->query('select s.id,s.name,s.groupid from tb_space as s
                                                    where s.status=1 and s.type=0 and s.id in (select spaceid from tb_employee
                                                    where status=1 and id=? ) ORDER BY s.createtime DESC LIMIT 5 offset 0',array($id));
        return $query->result_array();
    }
     ...
     * @return array
     */
    public function getOuterSpaceList($id)
    {
        $query=$this->db->query('select s.id,s.name,s.groupid from tb_space as s
                                                        where s.status=1 and s.type=1 and s.id in (select spaceid from tb_employee
                                                        where status=1 and id=? ) ORDER BY s.createtime DESC LIMIT 5 offset 0',array($id));
        return $query->result_array();
    }
    ...
    * @return  array
    */
    public function creatSpace($companyName,$spaceName,$user,$password,$name = '')
    {
        $this->db->trans_begin();
        //创建企业并获取ID
        ...
        //创建空间并获取ID
        $sql = 'INSERT INTO tb_space (name,companyid,createtime,status) VALUES (?,?,NOW(),1)';
        ...
        //创建空间用户并获取ID
        $sql = 'INSERT INTO tb_employee (spaceid,personid,email,roleid,name,status,createtime) VALUES (?,?,?,0,?,1,NOW())';
        $employeeId=$this->getInsertNewId($sql,array($spaceId,$personId,$user,$name));
        //插入应用列表
        $this->db->query('insert into tb_space_app (spaceid,appid)  select ?,id from tb_app where type=0 and status=1',array($spaceId));
    ...
    public function updateImgurl($path,$uid)
    {
        $this->db->trans_begin();
        $this->db->query('update tb_employee set imageurl=? WHERE id=?',array($path,$uid));
    ...
    public function addInSpace($email,$password,$spaceList, $name = '')
    {
        $this->db->trans_begin();
        ...
        //modified by bishenghua for no employeeId 2013/05/13
        if ($query->num_rows) {
        	$employeeId = $query->row()->id;
        } else {
        	$sql = 'INSERT INTO tb_employee (spaceid,email,roleid,name,status,createtime) VALUES (?,?,0,?,0,NOW())';
        	$employeeId = $this->getInsertNewId($sql, array($spaceList[0], $email, $name));
        }
        $data['employeeId'] = $employeeId;
        $query = $this->db->query('select e.id,e.spaceid,s.groupid from tb_employee as e left join tb_space as s on s.id = e.spaceid where e.email=? and e.status=0 and e.spaceid in ('.implode(',',$spaceList).')',array($email));
        $table = $query->result();
        $employeeList = [];
        $feedList = [];
        $i = 0;
        foreach($table as $row)
        {
            ...
            $feedList[$i]['eid'] = $row->id;
            $feedList[$i]['sid'] = $row->spaceid;
            $feedList[$i]['gid'] = $row->groupid;
            $i++;
        }
        $data['employeeList'] = $employeeList;
        ...
        $data['personId'] = $personId;
        $this->db->query('update tb_employee set status=1,createtime=NOW(),personid=? where id in ('.implode(',',$employeeList).')',array($personId));

        $object_type_qz = $this->config->item('object_type_qz', 'base_config');
        $ft_space_join = $this->config->item('ft_space_join', 'feed_config');
        foreach($feedList as $row)
        {
            $this->feed->addFeed($row['sid'], $row['eid'], $row['sid'], $object_type_qz, $ft_space_join, $row['gid']);
        }
    ...
    */
    ...
    /**
     * 翼商云登录 added by bishenghua 2013/05/10
     * @param unknown $email
     * @return boolean
     */
    public function esyLogin($email, $spaceid = 0) {
    	$data = $this->getUserInfo($email, $spaceid);
    	if (empty($data)) {
    		...
    		$this->input->set_cookie('session_id', $sessionId, 604800);
    		$this->input->set_cookie('qiater_autologin', '1', 604800);
    		$this->input->set_cookie('qiater_user', $this->encrypt($email), 604800);
