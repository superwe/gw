...
class Oauth_server
{
	...
	public function validate_client($client_id = '', $client_secret = NULL, $redirect_uri = NULL)
	{
		...
		$client_check_query = $this->ci->db
										...
										->get_where('tb_oauth_applications', $params);
	...
	public function new_auth_code($client_id = '', $user_id = '', $redirect_uri = '', $scopes = array(), $access_token = NULL)
	{
		...
		if ($access_token !== NULL)
		{
			...
			$this->ci->db
						...
						->update('tb_oauth_sessions', array(
							'code'			=> $code,
							'stage'			=> 'request',
		...
		else
		{
			...
			$this->ci->db
						->delete('tb_oauth_sessions', array(
							'client_id'		=> $client_id,
							'type_id'		=> $user_id,
							'type'			=> 'user'
						));
			...
			$this->ci->db
						->insert('tb_oauth_sessions', array(
							'client_id'		=>	$client_id,
			...
			foreach ($scopes as $scope)
			{
				...
				{
					$this->ci->db
								->insert('tb_oauth_session_scopes', array(
	...
	public function validate_auth_code($code = '', $client_id = '', $redirect_uri = '')
	{
		$validate = $this->ci->db
								...
								->get_where('tb_oauth_sessions', array(
									'client_id'		=> $client_id,
									'redirect_uri'	=> $redirect_uri, 
									'code'			=> $code
								));
	...
	public function get_access_token($session_id = '')
	{
		...
		if ($exists_query->num_rows() === 1)
		{
			...
			$this->ci->db
						...
						->update('tb_oauth_sessions', array(
							'code'	=>	NULL,
							'stage'	=>	'granted',
							'access_token'	=>	$access_token, //更新token值
							'last_updated'	=>	time(), //更新时间
						));
			
			//更新session scopes的access_token
			$this->ci->db
						->where(array('session_id' => $session_id))
						->update('tb_oauth_session_scopes', array(
								'access_token'	=>	$access_token
						));
		...
		else
		{
			...
			$this->ci->db
						->where(array('id' => $session_id))
						->update('tb_oauth_sessions', $updates);
			...
			if (count($scopes) > 0)
			{
				foreach ($scopes as $scope)
				{
					$scope_exists = $this->ci->db
												...
												->count_all_results('tb_oauth_session_scopes');
	...
	{
		// Your code here
		$query = $this->ci->db->select(array('id', 'password'))
					->get_where('tb_admin', array(
						'username' => $username,
					));
		if ($query->num_rows() === 1)
		{
			$row =  $query->row();
			return new user($row->id, $username, $password);
		}
		return false;
