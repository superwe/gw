...
(function($, YY, util){
    ...
    function DataTable(options) {
        ...
        if (typeof me.defaults === 'undefined') {
            ...
            fn.renderBody = function(callback) {
                ...
                if (typeof listData !== 'undefined') {
                    ...
                    // 当在主体中更新内容后，执行回调函数;
                    me.addBodyStr(body_str) && typeof callback==='function' && callback(listData);
                    ...
                    me.getRemoteData(function(d){
                        ...
                        if(table instanceof Array) {
                            // 增加主体列表内容;
                            body_str = me.concateBody(table);
                        }
                        if(!body_str) {
                            var len = 1;
                            if(typeof me.dataCache.head !== 'undefined') {
                                len = me.dataCache.head.length;
                            }
                            body_str = '<tr><td style="text-align: center;padding: 70px 0" colspan="'+len+'">没有任何数据</td></tr>';
                        }
                        // 当在主体中更新内容后，执行回调函数;
                        me.addBodyStr(body_str) && typeof callback==='function' && callback(d);
                        me.renderPage(pageCount);
                    });
                }
            };
            ...
            fn.renderPage = function(count) {
                var me = this;
                count = count || 5;
                var totalNum = parseInt(me.dataCache.totalNum),
                    perPage  = me.getPerPage(),
                    curPage  = me.curPage;
                if (!totalNum) {
                    me.removePage();
            ...
            fn.bindEvent = function() {
                ...
                var $selector = me.$selector,
                    options   = me.options,
                    $actLine  = $(options.actLine),
                    $pageLine = $actLine.find(options.pageLine);
                // 表格区域的点击事件；
                $selector.on({
                    click: function(e) {
                        var $target = $(e.target),
                ...
                $actLine.on({
                    click: function(e) {
                        ...
                        // 下一页;
                        if($target.is(nextBtn)) {
                            $pageLine.addClass('yy-click-locked');
                            util.trace(me.options.remoteUrl)
                        ...
                        // 直接根据页码翻页;
                        if($target.is(pageBtn)) {
                            $pageLine.addClass('yy-click-locked');
                            me.setToPage($target.text());
                            me.renderBody();
                        }
                        // 直接跳转到第一页;
                        if($target.is(firstBtn)) {
                            $pageLine.addClass('yy-click-locked');
                            me.setToPage(1);
                            me.renderBody();
                        }
                        // 直接跳转到最后一页;
                        if($target.is(lastBtn)) {
                            $pageLine.addClass('yy-click-locked');
        ...
        me.options.isRender && me.render(me.options.success);
    }
