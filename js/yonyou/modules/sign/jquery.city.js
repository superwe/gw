 ...
 * 插件名称: 中国省市信息联动插件
 */
$._cityInfo = [{"n":"北京市","c":["东城区","西城区","海淀区","朝阳区","丰台区","石景山区","通州区","顺义区","房山区","大兴区","昌平区","怀柔区","平谷区","门头沟区","密云县","延庆县"]},
{"n":"天津市","c":["和平区","河西区","南开区","河东区","河北区","红桥区","东丽区","津南区","西青区","北辰区","滨海新区","武清区","宝坻区","蓟县","宁河县","静海县"]},
{"n":"上海市","c":["黄浦区","徐汇区","长宁区","静安区","普陀区","闸北区","闵行区","宝山区","嘉定区","浦东新区","金山区","松江区","青浦区","奉贤区","崇明县","虹口区","杨浦区"]},
{"n":"重庆市","c":["渝中区","大渡口区","江北区","沙坪坝区","九龙坡区","南岸区","北碚区","綦江区","双桥区","渝北区","巴南区","万州区","涪陵区","黔江区","长寿区","江津区","合川区","永川区","南川区","綦江县","潼南县","铜梁县","大足县","荣昌县","璧山县","梁平县","城口县","丰都县","垫江县","武隆县","忠县","开县","云阳县","奉节县","巫山县","巫溪县","石柱土家族自治县","秀山土家族苗族自治县","酉阳土家族苗族自治县","彭水苗族土家族自治县"]},
...
{"n":"内蒙古自治区","c":["呼和浩特","阿拉善盟","包头","赤峰","东胜","海拉尔","集宁","临河","通辽","乌海","乌兰浩特","锡林浩特"]},
{"n":"新疆维吾尔自治区","c":["乌鲁木齐","阿克苏","阿勒泰","阿图什","博乐","昌吉","东山","哈密","和田","喀什","克拉玛依","库车","库尔勒","奎屯","石河子","塔城","吐鲁番","塔城","伊宁"]},
{"n":"宁夏回族自治区","c":["银川市","石嘴山市","吴忠市","固原市","中卫市"]}];
 
$.initProv = function(prov, city) {
    var provEl = $(prov);
    var cityEl = $(city);
    var defaultProv = provEl.attr('prov') ? provEl.attr('prov') : 0;
    var defaultCity = cityEl.attr('city') ? cityEl.attr('city') : 0;
    ...
    for(var i = 0; i < $._cityInfo.length; i++) {
        ...
        if(hasDefaultProv && $._cityInfo[i].n == defaultProv){
        	key = i;
        }
    }
    provEl.html(provHtml);
    $.initCities(provEl, cityEl, key, defaultCity);
    provEl.change(function() {
        ...
        $.initCities(provEl, cityEl, key);
    });
};
 
$.initCities = function(provEl, cityEl, key, defaultCity) {
    var hasDefaultCity = (typeof(defaultCity) != 'undefined');
    if(provEl.val() != '' && key >= 0) {
        var cities = $._cityInfo[key].c;
        var cityHtml = '';
...
/**
 * 页面显示呈现
 * @param string 省份显示的控件ID串 ex:"#prov"
 * @param string 城市显示的控件ID串 ex:"#city"
 */
$.initShow = function(provSpan, citySpan) {
	var ps = $(provSpan);
	var cs = $(citySpan);
	var pid = ps.attr('prov') ? ps.attr('prov') : 0;
	var cid = cs.attr('city') ? cs.attr('city') : 0;
    ps.html($._cityInfo[pid].n);
    cs.html($._cityInfo[pid].c[cid]); 
};