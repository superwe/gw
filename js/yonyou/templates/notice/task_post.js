(function($, YY, util){
     ...
     */
    notice.task_post = function(data){
        var ret = [],
            isnew = '',
        ...
        ret.push('<span class="gg_ico fl"></span><a target="_blank" href="/employee/myhomepage/index/'+data.authorid+'.html" tips="1" rel ="/employee/employee/cardInfo/'+data.authorid+'">'+data.authorname+'</a> 提交了任务"'+data.title+'"，请及时审核');
        ret.push('</div>');
        if(parseInt(data.status) == 5){
            ret.push('<div name="_t_'+data.id+'" class="clearfix content">');
            ret.push('    <p>');
            ret.push('    <input type="button" data="'+data.id+'" class="button button_blue yy-task-pass" value="通过">');
            ret.push('    <input type="button" data="'+data.id+'" class="button button_gray yy-task-unpass" value="驳回">');
            ret.push('    </p>');
            ret.push('</div>');
        }
        ...
        ret.push('                    <a target="_blank" href="/employee/myhomepage/index.html" tips="1" rel ="/employee/employee/cardInfo/'+data.creatorid+'"><img title="'+data.creatorname+'" alt="" src="http://static.yonyou.com/'+data.imageurl+'" onerror="imgError(this);" rel="http://static.yonyou.com/qz/default_avatar.thumb.jpg"></a>');
        ret.push('                  </figure>');
        ...
        if(userList.manage !== undefined){
            $.each(userList.manage, function(k, v){
                ret.push('<a href="/employee/myhomepage/index.html" tips="1" rel ="/employee/employee/cardInfo/'+k+'">',v,'</a>');
            });
        }
        ret.push('                               </li>');
