        ...
        ret.push('<div class="all container user_card likediv feed-section" resource-id="'+parseInt(data.feedid)+'">');
            ...
            ret.push('<div class="grid likeInfo" >');
                ret.push('<div style="font-size: 14px;">');
                    ret.push('<a style="color: #0178B3;" href="/employee/homepage/index/'+employeeId+'.html" tips="1" rel="'+'/employee/employee/cardInfo/'+employeeId+'">'+employeeName+'</a>收藏了发言');
                ret.push('</div>');
                $.each(likeList, function(i, item){
                    ret.push('<div class ="likeDetail">');
                        ret.push('在<a >'+item.groupname+'</a>中收藏了'+'<a>'+item.feedCreatorName+'</a>的发言: '+'<a>'+item.feedContent+'</a>');
                    ret.push('</div>');
